...
**Hosting:** Netlify, connected to GitHub (auto-deploys on push to `master`)  
...
**Host:** Netlify, connected to the GitHub repo [github.com/twolfrealestate/tommy-wolf-website](https://github.com/twolfrealestate/tommy-wolf-website). Netlify builds and deploys automatically on every push to `master` — there is no manual or drag-and-drop deploy step.
...
Netlify runs `npm run build` on its own servers after each push to `master` and publishes the result. There's nothing to build or upload locally to deploy — pushing to `master` is the deploy.
...
`public/sitemap.xml`, `public/llms.txt`, `public/llms-full.txt`, and `public/robots.txt` are generated by `scripts/generate-seo.mts`, which runs automatically via the `prebuild` npm script before every build. **Never hand-edit these four files** — the next build overwrites any manual change.

Blog posts, neighborhoods, and features are picked up automatically from their data files (`src/data/posts-meta.json`, `src/data/neighborhoods.ts`, `src/data/features.ts`). Any new **standalone page** (one that isn't a blog post, neighborhood, or feature) must be added to the `STANDALONE_PAGES` list inside `scripts/generate-seo.mts`, or it will be missing from all four generated files.

Because the generator runs in `prebuild` on Netlify's servers as part of each deploy, the generated files committed in the repo may lag behind the data files until the next deploy runs — this is expected, not a bug.
